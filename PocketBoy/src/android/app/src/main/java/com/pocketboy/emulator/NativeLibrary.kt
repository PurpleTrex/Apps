...
package com.pocketboy.emulator
 ...
 * with the native side of the PocketBoy code.
 */
object NativeLibrary {
    ...
    init {
        try {
            System.loadLibrary("pocketboy-android")
    ...
    val isPortraitMode: Boolean
        get() = PocketBoyApplication.appContext.resources.configuration.orientation ==
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val alert = MaterialAlertDialogBuilder(requireContext())
                ...
                .setMessage(
                    Html.fromHtml(
                        if (result == ErrorArticDisconnected)
                            PocketBoyApplication.appContext.resources.getString(R.string.artic_server_comm_error)
                        else
                            PocketBoyApplication.appContext.resources.getString(R.string.redump_games),
    ...
    /**
     * Logs the PocketBoy version, Android version and, CPU.
     */
    ...
    fun createFile(directory: String, filename: String): Boolean =
        if (FileUtil.isNativePath(directory)) {
            PocketBoyApplication.documentsTree.createFile(directory, filename)
        } else {
    ...
    fun createDir(directory: String, directoryName: String): Boolean =
        if (FileUtil.isNativePath(directory)) {
            try {
                PocketBoyApplication.documentsTree.createDir(directory, directoryName)
            } catch (e: Exception) {
                false
            }
        } else {
    ...
    fun openContentUri(path: String, openMode: String): Int =
        if (FileUtil.isNativePath(path)) {
            PocketBoyApplication.documentsTree.openContentUri(path, openMode)
        } else {
    ...
    fun getFilesName(path: String): Array<String?> =
        if (FileUtil.isNativePath(path)) {
            PocketBoyApplication.documentsTree.getFilesName(path)
        } else {
    ...
    fun getSize(path: String): Long =
        if (FileUtil.isNativePath(path)) {
            PocketBoyApplication.documentsTree.getFileSize(path)
        } else {
    ...
    fun fileExists(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            PocketBoyApplication.documentsTree.exists(path)
        } else {
    ...
    fun isDirectory(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            PocketBoyApplication.documentsTree.isDirectory(path)
        } else {
    ...
    fun copyFile(
        ...
        if (FileUtil.isNativePath(sourcePath) &&
            ...
            PocketBoyApplication.documentsTree
    ...
    fun renameFile(path: String, destinationFilename: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            try {
                PocketBoyApplication.documentsTree.renameFile(path, destinationFilename)
            } catch (e: Exception) {
                false
            }
        } else {
    ...
    fun deleteDocument(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            PocketBoyApplication.documentsTree.deleteDocument(path)
        } else {
