...
import com.pocketboy.emulator.databinding.ActivitySettingsBinding
...
class SettingsActivity : AppCompatActivity(), SettingsActivityView {
    ...
    fun onSettingsReset() {
        ...
        val editor =
            PreferenceManager.getDefaultSharedPreferences(PocketBoyApplication.appContext).edit()
        ...
        if (DirectoryInitialization.setPocketBoyUserDirectory()) {
            PocketBoyApplication.documentsTree.setRoot(Uri.parse(DirectoryInitialization.userPath))
            NativeLibrary.createConfigFile()
        } else {
            throw IllegalStateException("PocketBoy directory unavailable when accessing config file!")
        }

        // Set default values for system config file
        SystemSaveGame.apply {
            setUsername("POCKETBOY")
            setBirthday(11, 7)
            setSystemLanguage(1)
