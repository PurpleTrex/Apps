...
class UpdateUserDirectoryDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val choices = listOf(
            getString(R.string.keep_current_pocketboy_directory) to Uri.parse(cd).path,
            getString(R.string.use_prior_pocketboy_directory) to Uri.parse(ld).path
        )
        ...
        return MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(android.R.string.ok) { _: DialogInterface, _: Int ->
                if (selected == 1) {
                    PermissionsHandler.setPocketBoyDirectory(ld)
                }
                if (selected >= 0) {
                    PocketBoyDirectoryUtils.removeLimeDirectoryPreference()
                ...
                ViewModelProvider(mainActivity)[HomeViewModel::class.java].setUserDir(this.requireActivity(),PermissionsHandler.pocketboyDirectory.path!!)
            }
            .show()
    }
