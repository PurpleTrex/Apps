...
package com.pocketboy.emulator.adapters
...
import com.pocketboy.emulator.HomeNavigationDirections
...
import com.pocketboy.emulator.adapters.GameAdapter.GameViewHolder
import com.pocketboy.emulator.databinding.CardGameBinding
import com.pocketboy.emulator.databinding.DialogShortcutBinding
import com.pocketboy.emulator.features.cheats.ui.CheatsFragmentDirections
import com.pocketboy.emulator.fragments.IndeterminateProgressDialogFragment
import com.pocketboy.emulator.model.Game
import com.pocketboy.emulator.utils.FileUtil
import com.pocketboy.emulator.utils.GameIconUtils
...
class GameAdapter(private val activity: AppCompatActivity, private val inflater: LayoutInflater,  private val openImageLauncher: ActivityResultLauncher<String>?) :
    ...
    private fun gameExists(holder: GameViewHolder): Boolean {
        ...
        val gameExists = DocumentFile.fromSingleUri(
            PocketBoyApplication.appContext,
    ...
    private fun showOpenContextMenu(view: View, game: Game) {
        ...
        val popup = PopupMenu(view.context, view).apply {
            ...
            listOf(
                ...
                menu.findItem(id)?.isEnabled =
                    PocketBoyApplication.documentsTree.folderUriHelper(dir)?.let {
        ...
        popup.setOnMenuItemClickListener { menuItem ->
            ...
            val uri = when (menuItem.itemId) {
                R.id.game_context_open_app -> PocketBoyApplication.documentsTree.folderUriHelper(dirs.appDir)
                R.id.game_context_open_save_dir -> PocketBoyApplication.documentsTree.folderUriHelper(dirs.saveDir)
                R.id.game_context_open_updates -> PocketBoyApplication.documentsTree.folderUriHelper(dirs.updatesDir)
                R.id.game_context_open_dlc -> PocketBoyApplication.documentsTree.folderUriHelper(dirs.dlcDir)
                R.id.game_context_open_extra -> PocketBoyApplication.documentsTree.folderUriHelper(dirs.extraDir)
                R.id.game_context_open_textures -> PocketBoyApplication.documentsTree.folderUriHelper(dirs.texturesDir, true)
                R.id.game_context_open_mods -> PocketBoyApplication.documentsTree.folderUriHelper(dirs.modsDir, true)
                else -> null
            }
        ...
        popup.setOnMenuItemClickListener { menuItem ->
            val uninstallAction: () -> Unit = {
                when (menuItem.itemId) {
                    R.id.game_context_uninstall -> PocketBoyApplication.documentsTree.deleteDocument(dirs.gameDir)
                    R.id.game_context_uninstall_dlc -> FileUtil.deleteDocument(PocketBoyApplication.documentsTree.folderUriHelper(dirs.dlcDir)
                        .toString())
                    R.id.game_context_uninstall_updates -> FileUtil.deleteDocument(PocketBoyApplication.documentsTree.folderUriHelper(dirs.updatesDir)
                        .toString())
                }
                ViewModelProvider(activity)[GamesViewModel::class.java].reloadGames(true)
    ...
    private fun refreshShortcutDialogIcon() {
        if (imagePath != null) {
            val originalBitmap = BitmapFactory.decodeStream(
                PocketBoyApplication.appContext.contentResolver.openInputStream(
                    imagePath!!.toUri()
                )
            )
            val scaledBitmap = {
