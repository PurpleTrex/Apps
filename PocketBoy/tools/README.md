...
The scripts in this directory assume that your current working directory is the PocketBoy root directory and the script is being called via `./tools/xxx.sh`.
...
For consistency, when updating the translations to be committed to the repository, always perform the update within the the Docker environment we use for PocketBoy's CI (`opensauce04/pocketboy-build-environment:latest`).
